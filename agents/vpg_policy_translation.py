...
def env_fn(trajectory_logger):
    probe = Probe(
        pos=np.array([-20 / 1000, -5 / 1000, 0]), # only X and Y
        angle=0,
        width=40 / 1000,
        height=10 / 1000,
        focal_depth=10 / 1000
    ...
    env = FocalPointTaskUsEnv(
        ...
        probe_generator=RandomProbeGenerator(
            ...
            y_pos=np.arange(-25/1000, 29/1000, step=5/1000),
            focal_pos=[10/1000]
        ),
...
def cnn_actor_critic(x, a, training_ph=True, hidden_sizes=(64, 64),
                     activation=tf.tanh,
                     output_activation=None, policy=None, action_space=Discrete):
    ...
    # ------ Test, remove logp_all from return
